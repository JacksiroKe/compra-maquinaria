              ...
              <div class="card">
                <div class="card-header form-group">
                  ...
                  <div id="filter_list">
                    ...
                    <div class="filter_item" data-type="checkbox_modell" data-id="modellcheck_{{ $modell->id }}" style="display:none;">{{ $modell->name }} <i class="material-icons" style="font-size:16px;">close</i></div>
                    @endforeach
                  </div>
                </div>
                <div class="card-body filter_area">
                  ...
                  <!-- Model Filter -->
                  <div class="filter_eqmodell">
                    <a href="#eq_modell" data-toggle="collapse" role="button" aria-expanded="true" aria-controls="eq_modell">
                      <div class="collapse-header">
                        Model
                      </div>
                    </a>
                    <div class="collapse" id="eq_modell">
                      <div class="collapse-body">
...
@push('js')
  <script>
    ...
    $('.filter_item').click(function() {
      ...
      switch ($(this).attr('data-type')) {
        ...
        case 'checkbox_make':
          ...
          $(this).remove();
          break;
        case 'checkbox_modell':
          $('#'+$(this).attr('data-id')).prop('checked', false);
          sendAjaxRequestByModell();
          $(this).remove();
          break;
        default:
          break;
              ...
              // Modal Body
                ...
                // Row for general information
                  ...
                  modalHtml += "<div class='row'>";
                    ...
                    if (data[i].modell_id)
                      modalHtml += "<tr><td>Model</td><td>"+data[i].modell_name+"</td></tr>";
                    if (data[i].sn)
    ...
    function ajaxSendRequestByCategory() {
      ...
      $.ajax({
          ...
          success: function(res) {
            ...
            var modell_data = res.data_modell;
            var modell_modal_data = res.data_modell_modal;
            if (data == 'fail') {
            ...
            if (modell_data != 'fail') {
              $('#eq_modell .collapse-body').empty();
              for (var item in modell_data) {
                var strItem = `<div class="form-check">
                                <label class="form-check-label">
                                  <input class="form-check-input modell-filter-check" type="checkbox" unchecked id="modellcheck_${modell_data[item].id}" value="${modell_data[item].id}" onchange="ajaxSendRequestByMake();"> ${modell_data[item].name}
                                  <span class="form-check-sign">
                                    <span class="check"></span>
                                  </span>
                                </label>
                              </div>`;
                $('#eq_modell .collapse-body').append(strItem);
              }
              if (modell_modal_data != 'no_more'){
                var btnStr = `<div class="text-center">
                                <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modellModal"><i class="material-icons">add</i> Show All</button>
                              </div>`;
                $('#eq_modell .collapse-body').append(btnStr);
                $('#modellModal .modal-dialog .modal-content .modal-body .row').empty();
                for (var i in modell_modal_data) {
                  var strItem = `<div class="form-check col-md-3 col-sm-12">
                                  <label class="form-check-label">
                                    <input class="form-check-input modell-filter-check" type="checkbox" unchecked id="modellcheck_${modell_modal_data[i].id}" value="${ modell_modal_data[i].id }"> ${modell_modal_data[i].name}
                                    <span class="form-check-sign">
                                      <span class="check"></span>
                                    </span>
                                  </label>
    ...
    function sendAjaxRequestByMake() {
      ...
      $.ajax({
        type: "POST",
        url: "ajax_get_modells_by_makes",
        data: {
          makes: eq_make,
          _token: '<?php echo csrf_token(); ?>'
        },
        success: function(data) {
          $('#select_modell option').remove();
          $('#select_modell').append("<option value=''></option>");
          for(var item in data) {
              var category_itemId = data[item].id;
              $('#select_modell').append("<option value='"+data[item].id+"'>"+data[item].name+"</option>");
          }
          $('#select_modell').selectpicker('destroy').selectpicker();
        }
      });
      sendAjaxRequest();
    }

    function sendAjaxRequestByModell() {
      eq_model = [];
      $('#filter_list').find(`[data-type='checkbox_modell']`).hide();
      $(".modell-filter-check").each(function() {
        if ($(this).prop('checked') === true) {
          eq_model.push($(this).attr('value'));
          $('#filter_list').find(`[data-id='modellcheck_${$(this).attr('value')}']`).show();
        } 
      });
      sendAjaxRequest();
    }
